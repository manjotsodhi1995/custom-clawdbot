...
import {
  ...
  applyBlackboxRemoteCodeConfig,
  applyBlackboxRemoteCodeProviderConfig,
  BLACKBOX_REMOTE_CODE_DEFAULT_MODEL_REF,
...
export async function applyAuthChoiceApiProviders(
  ...
  if (authChoice === "blackbox-remote-code") {
    ...
    if (existingKey) {
      ...
      if (useExisting) {
        const result = await setClawdbotApiKey(existingKey, params.agentDir);
        await params.prompter.note(
          `Using existing CLAWDBOT_API_KEY from environment. Saved to ${result.path} for launchd compatibility.`,
          "CLAWDBOT API key",
        );
      } else {
        const key = await params.prompter.text({
          message: "Enter CLAWDBOT_API_KEY",
          validate: validateApiKeyInput,
        });
        const trimmed = normalizeApiKeyInput(String(key));
        const result = await setClawdbotApiKey(trimmed, params.agentDir);
        await params.prompter.note(
          `Saved CLAWDBOT_API_KEY to ${result.path} for launchd compatibility.`,
          "CLAWDBOT API key",
        );
      }
    } else {
      const key = await params.prompter.text({
        message: "Enter CLAWDBOT_API_KEY (required for remote-code tool)",
        validate: validateApiKeyInput,
      });
      const trimmed = normalizeApiKeyInput(String(key));
      const result = await setClawdbotApiKey(trimmed, params.agentDir);
    ...
    await params.prompter.note(
      "Blackbox Remote-Code provider enabled. The remote_code tool will be available when this model is selected.",
      "Blackbox Remote-Code",
    );
    {
      const applied = await applyDefaultModelChoice({
        config: nextConfig,
        setDefaultModel: params.setDefaultModel,
        defaultModel: BLACKBOX_REMOTE_CODE_DEFAULT_MODEL_REF,
        applyDefaultConfig: applyBlackboxRemoteCodeConfig,
        applyProviderConfig: applyBlackboxRemoteCodeProviderConfig,
        noteDefault: BLACKBOX_REMOTE_CODE_DEFAULT_MODEL_REF,
        noteAgentModel,
        prompter: params.prompter,
      });
      nextConfig = applied.config;
