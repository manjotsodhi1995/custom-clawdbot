...
export async function modelsStatusCommand(
  ...
  // Include implicit providers (e.g., blackbox-remote-code) that are always available
  const implicitProviders = await resolveImplicitProviders({ agentDir });
  const providersFromImplicit = new Set(
    Object.keys(implicitProviders ?? {})
      .map((p) => p.trim())
      .filter(Boolean),
  );
