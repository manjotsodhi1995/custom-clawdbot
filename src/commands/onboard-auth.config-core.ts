...
export const BLACKBOX_REMOTE_CODE_DEFAULT_MODEL_REF = "blackbox-remote-code/default";
...
export function applyBlackboxRemoteCodeConfig(cfg: ClawdbotConfig): ClawdbotConfig {
  const next = applyBlackboxRemoteCodeProviderConfig(cfg);
  const existingModel = next.agents?.defaults?.model;
  return {
    ...next,
    agents: {
      ...
      defaults: {
        ...
        model: {
          ...(existingModel && "fallbacks" in (existingModel as Record<string, unknown>)
            ...
            : undefined),
          primary: BLACKBOX_REMOTE_CODE_DEFAULT_MODEL_REF,
        },
      },
    },
  };
